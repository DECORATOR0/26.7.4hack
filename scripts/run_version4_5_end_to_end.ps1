...
$GoalFacts = "outputs_event_agent_v4_5_seed\scoreboard_goal_events.json"
...
function Write-Status {
    ...
    [ordered]@{
        ...
        frame_narration_output = "outputs_frame_narration_v4_5"
        memory_event_output = "outputs_event_agent_v4_5_text"
        scoreboard_goal_output = "outputs_event_agent_v4_5"
        script_report_output = "outputs_script_report_v4_5"
        model = "Intern S2"
        ...
        report_policy = "v4_5_split_items_markdown_and_delivery_markdown_deterministic"
...
Write-Status -Stage "version4_5_end_to_end" -Status "started"

Run-Step "scoreboard_goals_v4_5_seed_last_before_jump" @(
    ...
    "--out", "outputs_event_agent_v4_5_seed",
    "--precomputed-readings", $ScoreboardReadings,
    "--output-version", "v4_5_seed",
    "--goal-timestamp-policy", "last_before_jump",
    "--min-confidence", "0.5"
)

Run-Step "frame_narration_v4_5_goal_memory" @(
    "run_frame_narration.py",
    "--frame-index", "outputs_visual_full_safe\frame_index_2s.json",
    "--goal-memory", $GoalFacts,
    "--out", "outputs_frame_narration_v4_5",
    "--segment-seconds", "60",
    "--max-images", "30",
    "--concurrency", "8",
    "--rpm-limit", "15",
...
Run-Step "event_agent_v4_5_from_regenerated_narrative" @(
    "run_event_agent.py",
    "--descriptions", "outputs_frame_narration_v4_5\segment_descriptions.json",
    "--frame-index", "outputs_visual_full_safe\frame_index_2s.json",
    "--out", "outputs_event_agent_v4_5_text",
    "--chunk-segments", "12",
    "--concurrency", "3",
    "--rpm-limit", "12",
    "--temperature", "0.1",
...
Run-Step "guardrail_v4_5_text" @(
    "run_guardrail.py",
    "--events", "outputs_event_agent_v4_5_text\final_events.json",
    "--out", "outputs_event_agent_v4_5_text",
    "--temperature", "0",
    "--max-tokens", "6000"
)

Run-Step "scoreboard_goal_merge_v4_5_final" @(
    "run_scoreboard_goals.py",
    "--source-events", "outputs_event_agent_v4_5_text\final_events_guarded_v4.json",
    "--frame-index", "outputs_visual_full_safe\frame_index_2s.json",
    "--out", "outputs_event_agent_v4_5",
...
Write-Status -Stage "version4_5_end_to_end" -Status "done"
Write-Output "[$((Get-Date).ToString("o"))] VERSION4.5 END TO END DONE"
